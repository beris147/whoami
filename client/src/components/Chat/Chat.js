...
function Chat(props: ChatPropsT): React$Element<any> {
  ...
  const [textMessage: string | boolean, setTextMessage] = useState('');
  ...
  const handleYesOrNoAnswer = (answer: boolean) => {
  ...
  return (
    <div>
      ...
      {props.normal ? (
        ...
        <div>
          <button onClick={() => handleYesOrNoAnswer(YES)}>YES</button>
          <button onClick={() => handleYesOrNoAnswer(NO)}>NO</button>
        </div>
      )}
    </div>
  );
